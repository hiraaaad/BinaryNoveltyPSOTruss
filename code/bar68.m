function [best_feas_design,final_eval,total_eval] = bar68(M_upper)
kk = 1;
...
X_indep_ind=sort([(1:2:18)*2 (1:2:18)*2-1]); % coordinate variables that are not coupled to other coordinates
basic_node=[1 3 17:18]; %Basic nodes that must be present in the design
basic_member=[]; % members that must be active in design
X_const_ind=[1 2 5]; % indices of the fixed coordinates
X_const_val=[0 -80 0]; % values of the fixed coordinates

%
D_X = zeros(1,18*2);
U_X = zeros(1,18*2);

X = [0   0 0 120 120 120 240 240 240 360 360 360 480 480 480 600 600 600];
Y = [-40 0 40 -40 0 40 -40 0 40 -40 0 40 -40 0 40 -40 0 40];

X_shape = [2,4:1:18]*2 - 1;
Y_shape = [2,4:1:18]*2;
...
section_no=68; % The avilable section list for this problem. (see sections.m)
SF_buck=pi^2/kappa; 
Max_Kcond=1e14;
