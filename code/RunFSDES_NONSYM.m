% This is the source code of the improved fully stressed design evolution
%strategy (FSD-ES II), as published in [1]. Minor revisions were performed
% on the code for improving readability, which should not result in a change
% in the results. Use of this code is allowed upon acknowledgment and
% citation of [1]. The researchers are also recommended to review previous
% publications on FSD-ES [2,3]. For comments, feedback and questions,
% please email Ali Ahrari at aliahrari1983@gmail.com    

% References
% [1] Ahrari, Ali, and Kalyanmoy Deb. "An improved fully stressed design evolution strategy for layout optimization of truss structures." Computers & Structures 164 (2016): 127-144.
% [2] Ahrari, Ali, and Ali A. Atai. "Fully stressed design evolution strategy for shape and size optimization of truss structures." Computers & Structures123 (2013): 58-67.
% [3] Ahrari, Ali, Ali A. Atai, and Kalyanmoy Deb. "Simultaneous topology, shape and size optimization of truss structures by fully stressed design based on evolution strategy." Engineering Optimization 47, no. 8 (2015): 1063-1084.


function [best_feas_design,final_eval,total_eval] = RunFSDES_NONSYM(opt,kk,M_upper)
%Control parameetrs of FSD-ES II, which are  set to the default values.
%  clear;opt.initial_M=0.5;opt.topology_efficiency_check=0;opt.max_infeas=.5;opt.lambda_coeff=1;opt.mulambdaratio=.3;opt.maxeval_coeff=100;opt.Tccoeff=1;opt.Tscoeff=1;opt.c_sec_red=.05;opt.feastol=1e-7;kk=777;rng(kk)
 % opt.resize_budget=0;opt.inconly=0;opt.AdjustPcoeff=1;opt.GiveTimePcoeff=1;opt.tavan=2;
 % opt.TruncatedCompensation=0;opt.ExploitSymmetry=1;opt.UpdateLambda=0;opt.UpdateTau=[0 0];opt.updateSTR=[2 2 1];opt.updateZ=1; opt.ratiosUnsucc=[1 1 0]
%  opt.iniSMEAN=.25;opt.iniSTRMult=[1 1 1];opt.resizeStrategy=2;opt.TToplearn=1;opt.incAmeanTsTavan=inf
% opt.FSDBasedPenalty=0;opt.TerminateResizeNoChange=0;opt.DisConstMult=1; opt.WriteInterval=10;opt.AdjustMoveLimit=1;        opt.BiasIniAroundTavan=1;
%opt.ProblemName='960S'
 % ********************* Enter problem specific data here % **********************
 [opt,D,N_node,N_member,GSCP,sym_M,sym_A,X_indep_ind,basic_node,basic_member,X_const_ind,X_const_val,DOF_constrained,Fext,M_elasticity,Density,dis_all,SigT_all, SigC_all,kappa,Fy,section_no,Min_A,Max_A,D_X, U_X, minL,maxL]=GetProblemData_Hirad(opt);
...
if strcmp(opt.ProblemName,'10barDiscrete') || strcmp(opt.ProblemName,'10barDiscrete2') || strcmp(opt.ProblemName,'15barDiscrete') || strcmp(opt.ProblemName,'72barDiscrete') || strcmp(opt.ProblemName,'52barDiscrete') || strcmp(opt.ProblemName,'200barDiscrete')
    SF_buck=1e12;
end
...
%         end % The k-th solution that satisfies the necessary conditions for stability is generated
    ...
    Mmean(Mvar_ind)=YMEAN(N_Xvar+N_Avar+1:end);
      % update penalty coefficients (Pcoeff)
        ...
        %Pcoeff=Pcoeff.*exp( (infeas_ratio-opt.max_infeas)*opt.tau_Pcoeff ).*update_Pcoeff + Pcoeff .* (1-update_Pcoeff); % update Pcoeff
...
%     disp('   iter. No     #evals       best feas    best fval    max violation maxPcoeff maxzarib5'); disp([iter counteval best_feas_design(end) f(1)  max(constraint_vio(ind(1),:)) max(Pcoeff) max(zarib5) Max_Avar lambda])
    ...
    Ts=1/sqrt(2*opt.Tscoeff*N_var_eff);   % learning rate of the global step size
        ...
        % writing the optimization history and the best detected feasible solution at the specified intervals 
    if (toc-TimeInterval)>opt.WriteInterval
        ...
        %csvwrite(filename4,keep_best)
        save(datafile,'-v7.3','Xmean','Mmean','Amean','YMEAN','SMEAN','STR','iter','Pcoeff','best_feas_design','Max_Avar','counteval','keep_history','lambda','keep_best')
        TimeInterval=toc;
    end

%drawnow
 %[infeas_ratio(255:263); Pcoeff(255:263)]
 % check stopping criteria over last successive 50 iterations (Hirad)
 % keep record of last 50 
...
% csvwrite(filename4,keep_best)
% dlmwrite(filename3,best_feas_design','precision',12); % write the best detected feasible solution
% save(datafile,'-v7.3','Xmean','Mmean','Amean','YMEAN','SMEAN','STR','iter','Pcoeff','best_feas_design','Max_Avar','counteval','keep_history','lambda')
