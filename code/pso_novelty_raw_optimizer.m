function T = pso_novelty_raw_optimizer(id,benchmark,Particle_Number,Max_Gen,Radius_Novelty)
% parameters
Max_FES=1e6;
range=[0,1];
%
...
%
archive_novelty = [];
radius_novelty = Radius_Novelty; %k nearest neighbors
...
str_save = ['run_',num2str(benchmark),'bar/','run_local_',num2str(benchmark),'_',num2str(Particle_Number),'_',num2str(Max_Gen),'_',num2str(Radius_Novelty),'_iter_',num2str(id)];
% benchmark_ps_gen_radius
...
%         Max_Gen=10;
%         Particle_Number=5;        
        load('fitness_iterative_10bar.mat','Fitness')    
        DD = 2;
    case 11
        Dimension=10;
%         load('valid_topologies_10bar.mat')
...
%         Max_Gen=10;
%         Particle_Number=10;
        str_save = 'run_11bar_novelty_raw_iter_';
        DD = 2;
    case 15
        Dimension=15;
...
%         Max_Gen=50;
%         Particle_Number=5;
%         str_save = 'run_15bar_novelty_raw_iter_';
        load('fitness_iterative_15bar.mat','Fitness') 
        DD = 2;
    case 25
        Dimension=8;
%         load('valid_topologies_15bar.mat')
        load('25bar_info.mat','GSCP','basic_node','is_constrained_DOF','N_member','N_node','X_const_val','sym_M','DOF_constrained')
%         Max_Gen=10;
%         Particle_Number=5;
%         str_save = 'run_25bar_novelty_raw_iter_';
        DD = 3;
        load('fitness_iterative_25bar.mat','Fitness') 
        is_constrained_DOF=zeros(1,DD*N_node);
        is_constrained_DOF(DOF_constrained)=1;
    case 26
        Dimension=8;
%         load('valid_topologies_15bar.mat')
        load('26bar_info.mat','GSCP','basic_node','is_constrained_DOF','N_member','N_node','X_const_val','sym_M','DOF_constrained')
        Max_Gen=10;
        Particle_Number=10;
        str_save = 'run_26bar_novelty_raw_iter_';
        DD = 3;
        load('run_25bar_fitness.mat','F') 
        is_constrained_DOF=zeros(1,DD*N_node);
        is_constrained_DOF(DOF_constrained)=1; 
    case 52
        Dimension=12;
...
%         Max_Gen=100;
%         Particle_Number=30;
        load('fitness_iterative_52bar.mat','Fitness')  
        DD = 2;
    case 72
        Dimension=16;
...
%         Max_Gen=100;
%         Particle_Number=30;
%         str_save = 'run_72bar_novelty_raw_iter_';
        load('fitness_iterative_72bar.mat','Fitness')  
        DD = 3;
    case 73
        Dimension=16;
%         load('valid_topologies_72bar.mat')
        ...
        Max_Gen=100;
        Particle_Number=30;
        str_save = 'run_73bar_novelty_raw_iter_';
        DD = 3;
    case 200
        Dimension=29;
...
%         Max_Gen=100;
%         Particle_Number=30;
        str_save = 'run_200bar_novelty_raw_iter_';
        DD = 2;
end
...
[weight_archive,GenerationData] = keep_weight_archive_raw(pos,Fitness,weight_archive,id,GenerationData,gen);
           ...
           kp = key_p(X);
            ...
            if sum(f_x == 0) == 3 % feasible
                ...
                if ~isKey(feasible_upper,kp)
                    [weight_archive,GenerationData] = keep_weight_archive_raw(X,Fitness,weight_archive,id,GenerationData,gen);
                    fitcount  = fitcount+ 1;
                    TotalEval = 0;
                    feasible_upper(kp)=1; 
                    % add it to feasible 
